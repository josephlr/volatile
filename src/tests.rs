...
fn infer_new() {
    let x = 0u8;
    let _v1 = VolatilePtr::from_ref(&x);
    let mut y = 1u8;
    let _v2 = VolatilePtr::from_mut_ref(&mut y);
    let ptr: NonNull<u8> = (&y).into();
    unsafe {
        let _v3 = VolatilePtr::new(ptr);
        let _v4 = VolatilePtr::new_read_only(ptr);
        let _v5 = VolatilePtr::new_write_only(ptr);
    }
}
...
fn check_traits() {
    let mut v = 1u8;
    let p: NonNull<u8> = (&mut v).into();

    let vp = unsafe { VolatilePtr::new(p) };
    let vp1 = vp.with_access::<ReadOnly>();
    let vp2 = vp1.with_access::<ReadOnly>();
}
